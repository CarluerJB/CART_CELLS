...
class DENDROGRAM_TW:
    def __init__(self, save_dir_path):
        ...
        self.parameter_file_path = "PARAM_CLUST_DEFAULT.txt"
    ...
    def load_data(self):
        ...
        for id, elem in list_src["source"].items():
            ...
            self.source_go_term_df = pd.concat(
                [self.source_go_term_df, go_association.loc[missing_index,]]
            )
    ...
    def plot_clustering_TW(self):
        ...
        ax2.set_xticks(
            range(int(len(self.source_go_term_df.columns[:: self.X_POS_PER_PT])))
        )
